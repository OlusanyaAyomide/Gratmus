...
export default function UserInput() {
  ...
  const hanldleEmoji=(emoji:EmojiClickData)=>{
    setTextPost((prev=>`${prev}${emoji.emoji}`))
  }
  ...
  return (
    <div className='px-2'>
      ...
      <textarea  onChange={(e)=>{setTextPost(e.target.value)}}
       placeholder={imagePath?"Enter Caption":"Share thoughts"}
        onFocus={()=>{setPicked(false)}} 
        className={`outline-none mt-2 resize-none  rounded-lg border border-grat-light-text/10 py-2 home-scroll ${picked?"h-[80px]":"h-[180px] "} px-2 w-full`} value={postText}>
      </textarea>
