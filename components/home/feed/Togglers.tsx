    ...
    <div className={`px-1 w-full z-20 fixed xl:hidden bottom-1 sm:bottom-2  ${isToggled?"":"flex justify-end"}`}>
        ...
        <div className='w-full items-end flex flex-col'>
          ...
          <div className={`flex z-30 items-center md:hidden`}>
            ...
            <button  className='feed-buttons z-30 '><span className='text-grat-green'><BsPenFill/></span></button>
          </div>
       }
          {isToggled &&  FeedButtons.map((item,key)=>{
            return(
              <div className={`flex z-30 items-center ${key===1?"md:hidden":""}`}>
          ...
          {!isToggled &&<button onClick={()=>{setIsToggled(true);setOverLay(true)}} 
          className={`feed-buttons`}><span><FaPaperPlane/></span>
          </button>}
        </div> 
    </div>
  )
